...
res1 = zeros(length(temp_gamma), 1);
for j = 1:length(temp_gamma)
    for ii = 1:2
        cov = [sigma0^2 sigma0 * sigma1 * temp_gamma(j);
            sigma0 * sigma1 * temp_gamma(j) sigma1^2];
...
save("gamma_sensitivity.mat", "res1")
%% utility function
